...
min_input_voltage = 4700      # Minimum allowed input voltage
max_input_voltage = 5400      # Maximum allowed input voltage
...
def fetch_default_settings():
    ...
    try:
        ...
        default_frequency = system_info.get("frequency", 600)  # Fallback to 500 if not found
...
def restart_system():
    try:
        ...
        # If not an interrupt, wait 300s for system stabilization as some bitaxes are slow to ramp up
        if not is_interrupt:
            print(YELLOW + "Applying new settings and waiting 300s for system stabilization..." + RESET)
